...
handle_test_cases: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "expected_order": Order(
            action=CandleAction.SELL, entry=101.12, sl=101.38, tp=100.60, volume=17.857
        ),
        "expected_deposit": 1000,
        "expected_desicion_action": MarketAction.HOLD,
        "expected_desicion_order": Order(),
    ...
    {
        "name": "9. Not_ready -> off_maekwt_downtrend -> on_maekwt_downtrend-> cooldown. Candle triggers SL",
        "initial_trade_units": [
            Trade_unit(
                candle=Candle(
                    high=100.8, low=100.5, open=100.55, close=100.7, volume=1002
