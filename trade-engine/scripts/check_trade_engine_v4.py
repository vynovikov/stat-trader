...
from services.strategy_operator.reversal_strategy_operator_v4 import (
    ReversalStrategyOperatorV4,
)
from services.trade_engine.trade_engine_reversal_v4 import TradeEngineReversalV4
...
if __name__ == "__main__":
    ...
    reversal_strategy_operator = ReversalStrategyOperatorV4(leveraged_bookkeeper)
    ...
    trade_engine_v4 = TradeEngineReversalV4(
    ...
    for candle in candles:
        trade_unit = Trade_unit(
            candle=candle,
            spread=0.01,
            deposit=trade_engine_v4.parameters_store.deposit(),
        )
        decision = trade_engine_v4.handle_first(
            trade_unit=trade_unit,
        )
        if decision.action == MarketAction.CLOSE:
            print(
                f"{deal_number}. {decision.report.reason}. Deposit: {round(trade_engine_v4.parameters_store.deposit(),0)}"
            )
            ...
            # )
            trade_engine_v4.parameters_store.order_reset()
            trade_engine_v4.parameters_store.clear_report()
